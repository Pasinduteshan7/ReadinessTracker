...
interface AuthContextType {
  ...
  // Development-only helper to mock a social sign-in for local testing
  signInMock?: (provider: 'facebook' | 'linkedin') => Promise<void>;
  // Development-only helper to set an arbitrary dev user role
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const loadUser = async () => {
    try {
      // First, check localStorage for user data from login
      ...
      // Fall back to old auth method if no localStorage user
      let currentUser = await getCurrentUser();

      // In development, if there's an authenticated user but no profile/role
      // present in the DB, attempt to return a dev fallback synthetic user so
      // the frontend can run without email confirmations or profile rows.
      // eslint-disable-next-line @typescript-eslint/ban-ts-comment
      // @ts-ignore
      if (!currentUser && import.meta?.env?.DEV) {
        // Lazy import to avoid adding overhead in production builds
  ...
  // Development-only mock sign-in: set a fake user in the context so the app
  // can be tested without real OAuth provider redirects. Only available in dev.
  const signInMock = async (provider: 'facebook' | 'linkedin') => {
    if (!import.meta.env.DEV) {
      throw new Error('signInMock is only available in development');
    }

    // Create a deterministic fake user for local testing
