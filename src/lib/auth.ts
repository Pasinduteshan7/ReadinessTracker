...
// Development fallback: when running locally, allow users without profile rows
// to be treated as valid test users. This helps avoid email confirmation and RLS
// restrictions during frontend development. The logic below will only be used
// when import.meta.env.DEV is true.
export async function getCurrentUserDevFallback(): Promise<AuthUser | null> {
  const { data: { user } } = await backend.auth.getUser();
  if (!user) return null;

  // Try to get role/profile as usual
  const role = await getUserRole(user.id);
  ...
  // Only use the fallback in development
  // eslint-disable-next-line @typescript-eslint/ban-ts-comment
  // @ts-ignore - import.meta.env isn't typed here in Node tooling
